...
# Fixed system instruction for all examples (per your prompt)
SYSTEM_INSTRUCTION = (
    "My house is on fire now, I want to save as many valuable target objects as possible by picking them "
    "up and putting them into my bag. Given the current state and my previous actions, please help me choose "
    "the best available action to save as many valuable target objects as possible. All objects are denoted as "
    "<name> (id), such as <table> (712), and different objects have different values. Note objects lose their value "
    "once they start burning. The status of the objects previously seen may not be accurate any more. The final "
    "score is measured by the total value of the target objects I saved successfully."
)
...
def _split_system_user(prompt_text: str):
    """
    Always use the fixed system instruction and place the full prompt text into the user message.
    """
    system = SYSTEM_INSTRUCTION
    user = prompt_text.strip() if isinstance(prompt_text, str) else ""
    return system, user

def build_messages(entry, use_cot=False, cot_first=False):
    """
    Build chat-format messages: fixed system, user contains entire prompt text, assistant contains completion.
    """
    ...
    # fallback simple extraction if build_completion returned empty
    if not assistant_text:
        assistant_text = ""
        if entry.get("output") is not None:
            if isinstance(entry["output"], list):
                assistant_text = ", ".join(str(x) for x in entry["output"])
            else:
                assistant_text = str(entry["output"])
        if not assistant_text and entry.get("outputs"):
            if isinstance(entry["outputs"], list) and entry["outputs"]:
                assistant_text = str(entry["outputs"][0])
            else:
                assistant_text = str(entry["outputs"])
        if not assistant_text and entry.get("plan") is not None:
            p = entry["plan"]
            assistant_text = ", ".join(str(x) for x in p) if isinstance(p, list) else str(p)
        if not assistant_text and entry.get("cot_outputs"):
            assistant_text = entry["cot_outputs"][-1] if isinstance(entry["cot_outputs"], list) else str(entry["cot_outputs"])
...
def convert_dir(input_path: str, output_path: str, use_cot: bool = False, cot_first: bool = False,
    ...
    for f in files:
        ...
        # skip trivial placeholder examples that should not be included in training data
        if _is_placeholder_messages(j):
            # optional debug print:
            # print(f"Skipping placeholder file: {f}")
            continue
...
def _is_placeholder_messages(obj):
    """Return True for the exact placeholder: system='You are a helpful assistant.', user='' and assistant in {'N/A',''}."""
    if not isinstance(obj, dict):
        return False
    msgs = obj.get("prompts")
