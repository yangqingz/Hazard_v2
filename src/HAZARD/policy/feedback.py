import pdb
from git import Optional, Sequence, Tuple
import tiktoken
import openai
import json
import os
...
class Feedback:
                ...
                if source == 'openai':
                    ...
                    try:
                        if self.chat:
                            ...
                            if 'gpt-4' in self.lm_id or 'o1-preview' in self.lm_id:
    ...
    def evaluate_action(self,
        ...
        """
        Ask the LLM: “Does this action make sense? If not, suggest a better one.”

        Args:
            current_action: the action you just executed (e.g. "pick_up(obj3)").
            state_description: textual summary of object states & other context.
            available_actions: list of strings like "walk_to(obj1)", "drop(None)".
            task_instruction: your overall instruction (e.g. "Collect all fire-extinguishers").

        Returns:
            (True, None) if the LLM agrees the action makes sense;
            (False, suggestion) if it thinks there’s a better action.
        """
        # 1. build the prompt
        prompt = f"""
        Task Instruction:
        ...
        Question:
        Does the current action make sense given the task? 
        - If yes, reply “Yes.”
        - If no, reply “No.” Then on the next line, propose the single most reasonable action from the available list.
        """
        user_msg = [{"role": "user", "content": prompt}] if self.chat else prompt

        # 2. call the LLM
        outputs, _ = self.generator(user_msg, self.sampling_params)
        response = outputs[0].strip()

        # 3. parse
        lines = response.splitlines()
        decision = lines[0].lower().startswith("yes")
        suggestion = None
        if not decision and len(lines) > 1:
            # assume second line is the proposal
    ...
    def run(self,  holding_objects, nearest_object, object_list, action_history, agent_pos):
        info = {}
        ...
        # 1. Describe current state for prompt and feedback
        progress_desc = self.progress2text()
        
        # Prepare action history description
        action_history_desc = ", ".join(action_history[-10:] if len(action_history) > 10 else action_history)
        
         # 2. Build LLM prompt for next action decision
        ...
        # 3. Get available actions and plans
        ...
        # 4. Query LLM for next action
        if self.cot:
            prompt_full = prompt + " Let's think step by step."
            chat_prompt = [{"role": "user", "content": prompt_full}]
            outputs, usage = self.generator(chat_prompt if self.chat else prompt_full, self.sampling_params)
            output = outputs[0]
            ...
            normal_prompt = prompt_full + output + ' So the answer is'
            outputs, usage = self.generator(followup if self.chat else normal_prompt, self.sampling_params)
            output = outputs[0]
            self.total_cost += usage
            info['output_usage'] = usage
        else:
            chat_input = [{"role": "user", "content": prompt}] if self.chat else prompt
            outputs, usage = self.generator(chat_input, self.sampling_params)
            output = outputs[0]
            info['cot_usage'] = usage
            
            
        # 5. Parse and pick the suggested plan
        plan = self.parse_answer(available_plan_list, output)
        self.update_history(plan)
        chosen_action = actions[available_plan_list.index(plan)]

        # 6. Provide feedback and possibly override
        ok, suggestion = self.evaluate_action(
            current_action=f"{chosen_action[0]}({chosen_action[1]})",
            state_description=state_desc,
            available_actions=[f"{a[0]}({a[1]})" for a in actions],
            task_instruction="Complete the task safely."
        )
        
        
        if not ok and suggestion:
            # find matching tuple for suggestion
            try:
                idx = [f"{a[0]}({a[1]})" for a in actions].index(suggestion)
                chosen_action = actions[idx]
                plan = suggestion
                if self.debug:
                    print(f"Action overridden by feedback: {suggestion}")
            except ValueError:
                if self.debug:
                    print(f"Feedback suggestion '{suggestion}' not in available actions.")

        # 7. Package info and return
        if self.debug:
            print(f"plan: {plan}\n")
        info.update({
        ...
        return chosen_action, info, state_desc